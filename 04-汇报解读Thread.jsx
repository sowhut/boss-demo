...
function NewContactDialog({onClose,onCreate}){
  ...
  return<Overlay onClose={onClose}><div className="anim-slide" style={{width:520,maxHeight:"85vh",background:"#fff",borderRadius:20,boxShadow:"0 24px 80px rgba(0,0,0,.18)",overflow:"hidden",display:"flex",flexDirection:"column"}}>
    ...
    <div className="shrink-0 flex justify-end gap-3" style={{padding:"16px 24px",borderTop:`1px solid ${P.borderLight}`}}>
      ...
      <button onClick={()=>onCreate({name:name.trim(),relation,note:note.trim()})} disabled={!ok} style={{padding:"10px 28px",borderRadius:10,border:"none",cursor:ok?"pointer":"not-allowed",background:ok?P.ink:P.warmDark,color:ok?"#f5f5f5":P.inkFaint,fontSize:14,fontWeight:600,fontFamily:F,opacity:ok?1:0.6}}>确认创建</button>
    </div>
  </div></Overlay>;
}
...
function PersonConfirmMsg({onConfirm,showAnn}){
  ...
  const confirm=()=>{const name=selected?.name||newProfile?.name||"张总";setDone(true);onConfirm(name)};
      ...
      <Bubble><p style={{marginBottom:12}}>文档落款写的是「市场中心」，没有具名。这是谁的汇报？</p>
        {!done?<div>
          <div className="flex items-center gap-2 flex-wrap">
            {EXISTING_PEOPLE.map(p=><button key={p.id} onClick={()=>{setSelected(p);setNewProfile(null)}} className="flex items-center gap-1.5 transition-all duration-150" style={{padding:"6px 12px",borderRadius:20,fontSize:13,cursor:"pointer",fontFamily:F,border:`1px solid ${selected?.id===p.id?p.color:P.border}`,background:selected?.id===p.id?p.bgColor:"#fff",color:selected?.id===p.id?p.color:P.inkMuted,fontWeight:selected?.id===p.id?600:400}}>
            ...
            <button onClick={()=>setShowCreateDialog(true)} className="transition-all duration-150" style={{padding:"6px 12px",borderRadius:20,fontSize:13,cursor:"pointer",fontFamily:F,border:`1px solid ${newProfile?P.blue:P.border}`,background:newProfile?P.blueBg:"#fff",color:newProfile?P.blue:P.inkMuted,fontWeight:newProfile?600:400}}>都不是？新建联系人</button>
          </div>
          {newProfile&&<div className="anim-in" style={{marginTop:10,padding:"10px 12px",borderRadius:10,background:P.blueBg,border:`1px solid ${P.border}`}}><div style={{fontSize:12,color:P.blue,fontWeight:600,marginBottom:4}}>待创建联系人</div><div style={{fontSize:13,color:P.ink,fontFamily:F}}>{newProfile.name} · {newProfile.relation}</div>{newProfile.note&&<div style={{fontSize:12,color:P.inkMuted,marginTop:4,fontFamily:F}}>{newProfile.note}</div>}</div>}
          {(selected||newProfile)&&<button onClick={confirm} className="anim-in transition-all duration-150" style={{marginTop:10,padding:"6px 20px",borderRadius:20,border:"none",background:P.ink,color:"#f5f5f5",fontSize:13,fontWeight:500,cursor:"pointer",fontFamily:F}}>确认</button>}
        </div>
        :<div className="anim-in flex items-center gap-2" style={{padding:"6px 12px",borderRadius:10,background:P.greenBg}}><span style={{fontSize:11,color:P.green,fontWeight:500}}>✓ 已确认：{selected?.name||newProfile?.name}</span></div>}
      </Bubble>
    </div></div>{showCreateDialog&&<NewContactDialog onClose={()=>setShowCreateDialog(false)} onCreate={(profile)=>{setNewProfile(profile);setSelected(null);setShowCreateDialog(false)}}/>}
  </div>;
}
